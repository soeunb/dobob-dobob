...
alter table public.menu_templates
  add column if not exists household_id uuid references public.households(id) on delete cascade,
  add column if not exists menu_name text,
  add column if not exists slot text check (slot in ('breakfast', 'lunch', 'snack', 'dinner')),
  add column if not exists note text not null default '',
  add column if not exists storage_tags text[] not null default '{}'::text[],
  add column if not exists author_id uuid references public.profiles(id) on delete set null,
  add column if not exists created_at timestamptz not null default now(),
...
drop policy if exists "members can view meals" on public.meal_missions;
drop policy if exists "members can create meals" on public.meal_missions;
drop policy if exists "members can update meals" on public.meal_missions;
drop policy if exists "members can delete meals" on public.meal_missions;
...
drop policy if exists "members can view fridge memos" on public.fridge_memos;
drop policy if exists "members can create fridge memos" on public.fridge_memos;
drop policy if exists "members can update fridge memos" on public.fridge_memos;
drop policy if exists "members can delete fridge memos" on public.fridge_memos;
...
drop policy if exists "members can view templates" on public.menu_templates;
drop policy if exists "members can create templates" on public.menu_templates;
drop policy if exists "members can update templates" on public.menu_templates;
drop policy if exists "members can delete templates" on public.menu_templates;
